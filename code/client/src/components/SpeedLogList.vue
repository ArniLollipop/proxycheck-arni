...
<script>
import { getSpeedLogs } from '@/api/proxy.js';
  ...
  methods: {
    async fetchLogs() {
      ...
      try {
        ...
        const { data, total } = await getSpeedLogs(params);
        this.logs = data;
        this.totalRows = total;
      } catch (error) {
        alert('Failed to load speed logs.');
      } finally {
        this.isBusy = false;
      }
    },
    ...
    async loadProxies() {
      try {
        ...
        this.proxyOptions.push(...proxies.map(p => ({ value: p.id, text: p.name })));
      } catch (error) {
