...
.card.p-3
  ...
  b-form.d-flex.justify-content-start.flex-wrap.mb-3(@submit.prevent="applyFilters")
    ...
    b-form-group(label="Source IP", label-for="source-ip-filter", class="mr-2")
      b-form-input(id="source-ip-filter", v-model="filters.source_ip", size="sm", placeholder="Enter Source IP")
    b-form-group(label="Domain", label-for="domain-filter", class="mr-2")
      b-form-input(id="domain-filter", v-model="filters.domain", size="sm", placeholder="Enter Domain")
...
<script>
import { getProxyVisits, getProxies } from '@/api/proxy.js';

export default {
  name: 'ProxyVisitLogList',
  data() {
    return {
      logs: [],
      isBusy: false,
      ...
      fields: [
        ...
        { key: 'target_ip', label: 'Target IP', sortable: true },
  ...
  methods: {
    async fetchLogs() {
      ...
      try {
        ...
        const { data, total } = await getProxyVisits(params);
        this.logs = data;
        this.totalRows = total;
      } catch (error) {
        alert('Failed to load proxy visit logs.');
      } finally {
        this.isBusy = false;
      }
    },
